...
# This code pertains to the Andreev junction for the toy model on the square lattice described
# in Section III of the paper, and reproduces Figures 8, 9, and 10.
...
def make_system(**kwargs):
    keys = ['Nx','Ny', 'mu', 'tx', 'ty', 'Delta', 'phi', 'w']
    Nx, Ny, mu, tx, ty, Delta, phi, w = [kwargs.get(key) for key in keys]
...
filename = f'toy_junction_Nx_{Nx}_Ny_{Ny}_w_{w}_tx_{tx}_ty_{ty}_Delta_{Delta}_phi_{phi}_.hdf5'

## plot the system ##

print('-'*80, '\nplotting system - close window to start computation')

params = {'Nx': Nx, 'Ny': Ny, 'mu': 1.0, 'tx': tx, 'ty': ty, 'Delta': Delta, 'phi': phi, 'w': w}
...
plt.xlabel(r'$x/a$')
plt.ylabel(r'$y/a$')
# plt.savefig(f'{path_figs}/toy_junction_Nx_{Nx}_Ny_{Ny}_w_{w}_.png', dpi=1000, bbox_inches='tight', pad_inches=0.1)
plt.show()
...
for i in range(npar):
    ...
    params = {'Nx': Nx, 'Ny': Ny, 'mu': mulis[i], 'tx': tx, 'ty': ty, 'Delta': Delta, 'phi': phi, 'w': w}
    # add leads and finalize system
    syst, leads = make_system(**params)
    [syst.attach_lead(leads[n]) for n in range(4)]
    fsyst = syst.finalized()
