...
## Make plots for monolayer graphene ##

s = 12
Delta, phi = 1.0, 1.0
Lx, Ly, w = 300, 300, 20
# zigzag
filename = f'{path_data}/graphene_zigzag_s_{s}_Lx_{Lx}_Ly_{Ly}_w_{w}_Delta_{Delta}_phi_{phi}_.hdf5'
print('zigzag graphene junction')
# armchair
# filename = f'{path_data}/graphene_armchair_s_{s}_Lx_{Lx}_Ly_{Ly}_w_{w}_Delta_{Delta}_phi_{phi}_.hdf5'
# print('armchair graphene junction')
...
for i in range(npar):
    ...
    plt.plot(xdata1, g11[i], label = f'$\\mu = {mulis[i]} \\text meV$')

plt.title(f'$\\Delta_0 = {Delta} \\text meV \\quad (L_x/sa,L_y/sa) = ({Lx},{Ly})$')
...
for i in range(npar):
    ...
    plt.plot(xdata0, dg11[i], label = f'$\\mu = {mulis[i]} \\text meV$')
...
for i in range(npar):
    ...
    plt.plot(xdata0, dg21[i], label = f'$\\mu = {mulis[i]} \\text meV$')
