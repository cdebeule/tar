...
# File : graphene_armchair_scaled.py
...
def make_system(**kwargs):
    ...
    # x axis : armchair
    # y axis : zigzag
    lat = kwant.lattice.general([(0, 1), (np.sqrt(3) / 2, 1 / 2)], [(1 / (2 * np.sqrt(3)), 1 / 2), (-1 / (2 * np.sqrt(3)), 1 / 2)], norbs=2)
    a, b = lat.sublattices
    syst = kwant.Builder()
    ...
    sym_up.add_site_family(a, other_vectors=[(-1, 2)])
    sym_up.add_site_family(b, other_vectors=[(-1, 2)])
    sym_down.add_site_family(a, other_vectors=[(-1, 2)])
    sym_down.add_site_family(b, other_vectors=[(-1, 2)])
...
Lx, Ly, w = 300, 300, 30
filename = f'graphene_armchair_s_{s}_Lx_{Lx}_Ly_{Ly}_w_{w}_Delta_{Delta}_phi_{phi}_.hdf5'

# force vertical bearded edge
Nx = round((np.sqrt(3) * Lx + 2) / 3)
Lx = ( 3 * Nx - 2 ) / np.sqrt(3)
